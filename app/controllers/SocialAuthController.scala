      ...
      case Some(p: SocialProvider with CommonSocialProfileBuilder) =>
        p.authenticate().flatMap {
          ...
          case Right(authInfo) => for {
            ...
            _ <- userRepository.create(profile.loginInfo.providerID, profile.fullName getOrElse "brak danych", profile.loginInfo.providerKey, profile.email.getOrElse(""))
            _ <- authInfoRepository.save(profile.loginInfo, authInfo)
            authenticator <- authenticatorService.create(profile.loginInfo)
            value <- authenticatorService.init(authenticator)
            result <- authenticatorService.embed(value, Redirect("http://localhost:3000/searchproducts"))
          } yield {
